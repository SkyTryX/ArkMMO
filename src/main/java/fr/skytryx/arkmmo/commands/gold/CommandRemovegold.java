...
public class CommandRemovegold implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 2 && Ftion.isNumeric(args[1])) {
            ...
            arkPlayer.save();
            sender.sendMessage(Ftion.msgf("Gold", "Removed §6"+args[1]+"§b gold to §6" + arkPlayer.getName() + "§b. They now have §6" + arkPlayer.getGold()+" §b gold"));
        }
        return false;
    }
}
